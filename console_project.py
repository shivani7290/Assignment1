...
def logout(cursor, conn, current_user):
    if not current_user[0]:
        print("no user logged in currently")
        return
...
def change_password(cursor, conn, current_user):
    if not current_user[0]:
        print("first log in to change password")
        return

    old_pw = getpass.getpass("enter old password ").strip()
    new_pw = getpass.getpass("enter new password").strip()
...
def main():
    ...
    while True:
        ...
        print("5. Exit")
        choice = input("Select an option from (1-5): ").strip()
